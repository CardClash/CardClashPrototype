...
public class PhotonFighterScript : MonoBehaviourPun, IPunObservable
{
    #region Variables
    [HideInInspector]
    public SyncPlayer sync;
    public struct SyncPlayer
    {
        [HideInInspector]
        public int lives;
    ...
    #region Properties (Get/Set functions)

    public bool FacingRight
    {
        get { return sync.facingRight; }
        set { sync.facingRight = value; }
    }

    public bool Corrected
    {
        get { return sync.corrected; }
        set { sync.corrected = value; }
    }
    ...
    public int PlayerState
    {
        get { return sync.playerState; }
        set { sync.playerState = value; }
    }
    ...
    public int Lives
    {
        get { return sync.lives; }
        set { sync.lives = value; }
    }
    ...
    public float TimeStopTimer
    {
        get { return sync.timeStopTimer; }
        set { sync.timeStopTimer = value; }
    }
    ...
    public int OpponentDamage
    {
        get { return sync.opponentDamage; }
        set { sync.opponentDamage = value; }
    }
    ...
    void Start()
    {
        ...
        sync.facingRight = false;
        sync.corrected = false;

        defaultTime = Time.timeScale;
        sync.timeStopTimer = 0.0f;
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            stream.SendNext(sync.lives);
            stream.SendNext(sync.playerState);
            stream.SendNext(sync.facingRight);
            stream.SendNext(sync.corrected);
            stream.SendNext(sync.timeStopTimer);
            stream.SendNext(sync.artArrayNum);
            stream.SendNext(sync.opponentDamage);
        }
        else
        {
            sync.lives = (int)stream.ReceiveNext();
            sync.playerState = (int)stream.ReceiveNext();
            sync.facingRight = (bool)stream.ReceiveNext();
            sync.corrected = (bool)stream.ReceiveNext();
            sync.timeStopTimer = (float)stream.ReceiveNext();
            sync.artArrayNum = (int)stream.ReceiveNext();
            sync.opponentDamage = (int)stream.ReceiveNext();
        }   
    }       
}           
